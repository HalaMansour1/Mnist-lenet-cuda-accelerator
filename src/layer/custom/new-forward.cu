...
//#define Mask_map_out 16
//#define channel_out 4
//#define mask_width 7 
//__constant__ float const_mask[Mask_map_out*channel_out*mask_width*mask_width] ;  
__global__ void conv_forward_kernel(float *output, const float *input, const float *mask, const int Batch, const int Map_out, const int Channel, const int Height, const int Width, const int K)
{
   ...
   // (void)Width_out; // silence declared but never referenced warning. remove this line when you start working
    ...
    float acc = 0.0;
    if (h < Height_out && w < Width_out && n < Batch && m < Map_out){
    for (c = 0; c < Channel; c++) { // sum over all input channels
