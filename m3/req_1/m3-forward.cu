                                     ...
                                     int numCRows, int numCColumns)
{
    ...
    wmma::fragment<wmma::accumulator, 16, 16, 16, float> out_f;

    wmma::fill_fragment(out_f, 0.0f);

    __shared__ float sharedA[TILE_WIDTH][TILE_WIDTH];
    __shared__ float sharedB[TILE_WIDTH][TILE_WIDTH];
    __shared__ float sharedOut[TILE_WIDTH][TILE_WIDTH];
    ...
    for (int tileId = 0; tileId < (numAColumns - 1) / TILE_WIDTH + 1; tileId++) {
        if (row < numARows && tileId * TILE_WIDTH + tx < numAColumns) {
            sharedA[threadIdx.y][threadIdx.x] = __float2half(A[(size_t) row * numAColumns + tileId * TILE_WIDTH + tx]);
        } else {
            sharedA[threadIdx.y][threadIdx.x] = __float2half(0.0f);
        }
        if (col < numBColumns && tileId * TILE_WIDTH + ty < numBRows) {
            sharedB[threadIdx.y][threadIdx.x] = __float2half(B[(size_t) (tileId * TILE_WIDTH + ty) * numBColumns + col]);
        } else {
            sharedB[threadIdx.y][threadIdx.x] = __float2half(0.0f);
        }
        __syncthreads();

        if (threadIdx.y == 0 || threadIdx.y == 1){  
            wmma::load_matrix_sync(a_f, (half*)sharedA, TILE_WIDTH);
            wmma::load_matrix_sync(b_f, (half*)sharedB, TILE_WIDTH);
            wmma::mma_sync(out_f, a_f, b_f, out_f);
        }
        __syncthreads();
    }

    if (threadIdx.y == 0 || threadIdx.y == 1){
        wmma::store_matrix_sync((float*)sharedOut, out_f, TILE_WIDTH, wmma::mem_row_major);
    }
   // __syncthreads();
    
    if (row < numCRows && col < numCColumns) {
        C[(size_t) row * numCColumns + col] = sharedOut[threadIdx.y][threadIdx.x];
    }
}
