...
#define wbCheck(stmt)                                                     \
  ...
  __global__ void halfkernel(const float * device_mask, const int size, half * m_h){
      int i = blockIdx.x * blockDim.x + threadIdx.x;
      if (i < size){
      m_h[i] = __float2half(device_mask[i]);
      }
  }
    ...
    if (idx < Channel * W_unroll) {
        ...
        for (size_t p = 0; p < K; p++) {   
            for (size_t q = 0; q < K; q++) { 
                ...
                float val = in_4d(b, c, p + h_out, w_out + q);
                // Convert float to half inline
                output[h_unroll * W_unroll * Batch + W_unroll * b + w_unroll] = __float2half(val);
            }
        }
    }
...
int M_size = Map_out * Channel * K * K;
half * device_half;
cudaMalloc((void**)&device_half, M_size * sizeof(half));
...
dim3 grid(ceil(M_size/ (BLOCK_SIZE*1.0)), 1, 1);
dim3 block(BLOCK_SIZE, 1, 1);
halfkernel<<<grid, block>>>(device_mask, M_size, device_half);
...
matrixMultiplyShared<<<grid_matmul_dim, block_dim>>>(device_half, unrolled_matrix, matmul_output, mask_height, mask_width, Height_unrolled, Width_unrolled, mask_height, Width_unrolled);
